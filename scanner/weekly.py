"""Weekly strength scanner."""

import logging
from typing import Optional
import pandas as pd
import numpy as np

from scanner.indicators import Indicators
from services.market_data import MarketDataService
from config import Config
from models.stock import Indicators as IndicatorsModel, ScannerResult

logger = logging.getLogger(__name__)


class WeeklyScanner:
    """Weekly strength scanner."""

    def __init__(self, config: Config):
        """Initialize weekly scanner."""
        self.config = config
        self.market_data = MarketDataService()

    def scan(self, df: pd.DataFrame, symbol: str) -> ScannerResult:
        """
        Scan stock for weekly strength conditions.

        Args:
            df: Daily OHLCV DataFrame.
            symbol: Stock symbol.

        Returns:
            ScannerResult with conditions met.
        """
        result = ScannerResult(symbol=symbol, price=0.0, weekly=False)

        try:
            if df is None or len(df) < 50:
                logger.warning(f"Insufficient data for {symbol} weekly")
                return result

            # Resample to weekly
            weekly_df = MarketDataService.resample_weekly(df)

            if len(weekly_df) < 10:
                logger.warning(f"Insufficient weekly data for {symbol}")
                return result

            # Get latest values
            current_close = weekly_df["close"].iloc[-1]
            current_volume = weekly_df["volume"].iloc[-1]

            # Calculate indicators
            ema20 = Indicators.ema(weekly_df["close"], self.config.EMA_SHORT).iloc[-1]
            rsi = Indicators.rsi(weekly_df["close"], self.config.RSI_PERIOD).iloc[-1]
            avg_volume = Indicators.average_volume(
                weekly_df["volume"], self.config.VOLUME_AVG_PERIOD
            ).iloc[-1]

            # Check conditions
            condition1 = current_close > ema20
            condition2 = rsi > self.config.WEEKLY_RSI_MIN
            condition3 = Indicators.higher_highs(
                weekly_df, self.config.WEEKLY_SWING_COUNT
            )
            condition4 = Indicators.higher_lows(
                weekly_df, self.config.WEEKLY_SWING_COUNT
            )

            all_conditions_met = all(
                [condition1, condition2, condition3, condition4]
            )

            volume_ratio = Indicators.volume_ratio(current_volume, avg_volume)
            high_52w = df["high"].tail(252).max()
            distance_from_high = Indicators.distance_from_52w_high(
                current_close, high_52w
            )

            indicators = IndicatorsModel(
                symbol=symbol,
                ema20=float(ema20),
                rsi=float(rsi),
                average_volume=float(avg_volume),
                volume_ratio=float(volume_ratio) if volume_ratio else None,
                high_52w=float(high_52w),
                distance_from_52w_high=float(distance_from_high)
                if distance_from_high
                else None,
            )

            result = ScannerResult(
                symbol=symbol,
                price=float(current_close),
                weekly=all_conditions_met,
                indicators=indicators,
            )

            logger.info(f"{symbol} Weekly: {all_conditions_met}")
            return result

        except Exception as e:
            logger.error(f"Error scanning {symbol} weekly: {e}", exc_info=True)
            result.error = str(e)
            return result
